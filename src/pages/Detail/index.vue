...
export default {
  ...
  methods: {
    //产品的售卖属性值切换高亮,这里运用到了排他的思想，即点亮一处，熄灭余处
    ...
    changeSkuNum(event) {
      //用户输入进来的文本 * 1
      //比如"loveyou" * 1 会显示 NAN, 由此可以判断用户输入的是否是一个数字
      let value = event.target.value * 1;
      //如果用户输入进来的非法,出现NaN或者小于1
      if (isNaN(value) || value < 1) {
        this.skuNum = 1;
      } else {
        //正常大于1【大于1整数不能出现小数】parseInt转为整数
    ...
    async addShopcar() {
      ...
      //它是在调用vuex仓库中的这个addOrUpdateShopCart函数。这个方法加上async,返回的一定是一个Promise对象
